...
# Swap these out for the actual field origin before flying.
ORIGIN_LAT =  47.397742   # example: ArduPilot SITL default
ORIGIN_LON =   8.545594
...
def make_lawnmower_mission(
    width_m=60, height_m=40, strip_spacing_m=10, alt=15.0
) -> List[Waypoint]:
    """
    ...
    while y <= height_m:
        ...
        lat_a, lon_a = _offset(ORIGIN_LAT, ORIGIN_LON, y, x_start)
        lat_b, lon_b = _offset(ORIGIN_LAT, ORIGIN_LON, y, x_end)
...
# Manual + automated hybrid example:
# First two waypoints are "fly here manually inspected", then switch to lawnmower.
def make_hybrid_mission() -> List[Waypoint]:
    manual_legs = [
        Waypoint(*_offset(ORIGIN_LAT, ORIGIN_LON, 10, 10), alt=12.0, name="Manual WP 1"),
        Waypoint(*_offset(ORIGIN_LAT, ORIGIN_LON, 20, 10), alt=12.0, name="Manual WP 2"),
    ]
    automated_legs = make_lawnmower_mission(width_m=40, height_m=20,
                                            strip_spacing_m=8, alt=12.0)
    return manual_legs + automated_legs
...
def main():
    # ── 1. Choose your mission here ──────────────────────────────────────────
    #
    # Swap out whichever pattern you want to fly, or build your own list:
    #
    #   waypoints = make_box_mission(side_m=30, alt=10.0)
    #   waypoints = make_lawnmower_mission(width_m=60, height_m=40, alt=15.0)
    #   waypoints = make_refined_search_mission(ORIGIN_LAT, ORIGIN_LON, radius_m=20)
    #   waypoints = make_hybrid_mission()
    #
    # Or define waypoints manually:
    #
    #   waypoints = [
    #       Waypoint(47.397842, 8.545594, 10.0, name="Start"),
    #       Waypoint(47.398100, 8.546000, 10.0, name="Middle"),
    #       Waypoint(47.397742, 8.546200, 10.0, name="End"),
    #   ]
    #
    waypoints = make_box_mission(side_m=30, alt=10.0)
